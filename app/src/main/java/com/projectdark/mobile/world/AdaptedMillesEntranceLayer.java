...
public final class AdaptedMillesEntranceLayer {
  ...
  private static List<Entrance> build(){
    ...
    for(AdaptedMillesMapLayer.Structure s:AdaptedMillesMapLayer.structures()){
      ...
      if(iso!=null){r.add(new Entrance("entrance_"+s.id,s.id,iso.doorFootX,iso.doorFootY,28f*AdaptedMillesIsoBuildingLayer.B1_BUILDING_SCALE,iso.approachX,iso.approachY));continue;}
