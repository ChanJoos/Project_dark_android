...
public final class ReagentShopInteriorRuntime implements WorldMoveTargetController.NavigationWorld,WorldMoveTargetController.Walker {
  ...
  private float x=ReagentShopInteriorDef.SPAWN_X,y=ReagentShopInteriorDef.SPAWN_Y; private boolean leftSpawn;
  ...
  public void resetAtEntry(){movement.cancel();WorldMoveTargetController.TileCenter t=nearest(ReagentShopInteriorDef.SPAWN_X,ReagentShopInteriorDef.SPAWN_Y);x=t.x;y=t.y;leftSpawn=false;}
  ...
  public boolean atExit(){return leftSpawn&&ReagentShopInteriorDef.exitContains(x,y);}
  ...
  public boolean moveToAdjacentTile(float dx,float dy,WorldMoveTargetController.Direction d){
    if(WorldMoveTargetController.Direction.between(x,y,dx,dy)!=d||!canPlayerOccupy(dx,dy))return false;x=dx;y=dy;if(!ReagentShopInteriorDef.exitContains(x,y))leftSpawn=true;return true;
  }
