...
/**
 * Regression audit for the [ADAPTED]/[B] Milles prototype.
 * It mirrors RuntimeState player occupancy (blockers + NPC + monster fixtures) and 4-neighbour navigation.
 */
public final class WorldExplorationOccupancyAudit {
  ...
  public static boolean verify(){
    ...
    for(WorldExplorationContract.Anchor a:WorldExplorationContract.millesPrototype()){
      if(!occupiable(world,a.x,a.y))return false;
    }
    WorldExplorationContract.Anchor spawn=WorldExplorationContract.byId("spawn");
    ...
    long start=key(sx,sy);seen.add(start);q.add(start);
    ...
    while(!q.isEmpty()){
      ...
      for(int i=0;i<4;i++){
        ...
        long nk=key(nx,ny);if(seen.contains(nk))continue;
        if(!occupiable(world,cellX(nx),cellY(ny)))continue;
        seen.add(nk);q.addLast(nk);
      }
    }

    for(WorldExplorationContract.Anchor a:WorldExplorationContract.millesPrototype()){
      if(!reachableNear(seen,a))return false;
    }
    return true;
  }
  ...
  private static boolean occupiable(WorldDef world,float x,float y){
    ...
    for(WorldDef.NpcSpawn n:world.npcSpawns()){
      float min=r+RuntimeState.NPC_RADIUS+2f;if(distance(x,y,n.x,n.y)<min)return false;
    }
    for(WorldDef.MonsterSpawn m:world.monsterSpawns()){
      float min=r+RuntimeState.MONSTER_RADIUS+3f;if(distance(x,y,m.x,m.y)<min)return false;
    }
    return true;
  }
  ...
  private static boolean reachableNear(Set<Long> seen,WorldExplorationContract.Anchor a){
    ...
    for(int x=cx-range;x<=cx+range;x++)for(int y=cy-range;y<=cy+range;y++){
      if(seen.contains(key(x,y))&&distance(cellX(x),cellY(y),a.x,a.y)<=TARGET_TOLERANCE)return true;
    }
    return false;
  }

  private static int nearestCellX(float x){return Math.round((x-WorldDef.MIN_X)/GRID);}
  private static int nearestCellY(float y){return Math.round((y-WorldDef.MIN_Y)/GRID);}
  private static float cellX(int x){return WorldDef.MIN_X+x*GRID;}
  private static float cellY(int y){return WorldDef.MIN_Y+y*GRID;}
