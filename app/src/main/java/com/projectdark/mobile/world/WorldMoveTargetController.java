...
public final class WorldMoveTargetController {
  ...
  private static final int MIN_EXPANSIONS=4096,MAX_EXPANSIONS=65536,MAX_REPLANS=2;
  ...
  public Snapshot tick(float dt){if(status!=Status.MOVING||dt<=0f)return snapshot();if(distance(walker.worldX(),walker.worldY(),targetX,targetY)<=tolerance){status=Status.REACHED;return snapshot();}if(waypointIndex>=path.size()&&!rebuildPath()){status=Status.BLOCKED;return snapshot();}float[] wp=path.get(waypointIndex);float dx=wp[0]-walker.worldX(),dy=wp[1]-walker.worldY();float d=(float)Math.sqrt(dx*dx+dy*dy);if(d<=Math.max(1f,cellSize*.2f)){waypointIndex++;return tick(dt);}float step=Math.min(walkSpeed*dt,d);if(!walker.tryWalkStep(dx/d*step,dy/d*step)){replans++;if(replans>MAX_REPLANS||!rebuildPath())status=Status.BLOCKED;}else replans=0;if(distance(walker.worldX(),walker.worldY(),targetX,targetY)<=tolerance)status=Status.REACHED;return snapshot();}
  ...
  private int expansionLimit(){long columns=(long)Math.ceil((world.maxX()-world.minX())/cellSize)+1L;long rows=(long)Math.ceil((world.maxY()-world.minY())/cellSize)+1L;long cells=columns*rows;return (int)Math.max(MIN_EXPANSIONS,Math.min(MAX_EXPANSIONS,cells));}
