...
public final class MonsterAutoCombatBridgeAudit {
  private static final class Port implements CombatResolver.Port {
    ...
    public CombatResolver.EffectResult applyDamage(String actor,String target,String actionId,int amount){
      damageCalls++;
      int before=hp.get(target),after=Math.max(0,before-amount);hp.put(target,after);
      CombatResolver.DefeatedTargetKind kind=target.startsWith("monster")
  ...
  public static boolean verify(){
    ...
    // Manual and AUTO coexist in the same resolver; bridge itself cannot apply damage.
    ...
    // Unknown action remains explicit and mutation-free.
    ...
    return unresolved.outcome==MonsterAutoCombatBridge.Outcome.ACTION_UNRESOLVED&&
        port.damageCalls==before&&!resolver.actionActive("monster2");
  }

  private static CombatResolver.Definition attack(String id,int damage,float hitTime){
    return new CombatResolver.Definition(id,CombatResolver.ActionKind.ATTACK,CombatResolver.ActionState.ATTACK,
        CombatResolver.EffectType.PHYSICAL_HIT,false,0,.5f,50f,hitTime,damage);
  }
  private static boolean has(List<CombatResolver.Event> events,String actor,String actionId,
      CombatResolver.InputMode mode,CombatResolver.EventType type){
    for(CombatResolver.Event e:events)if(type==e.type&&actor.equals(e.actorId)&&actionId.equals(e.actionId)&&mode==e.inputMode)return true;
    return false;
  }
  public static void main(String[] args){
    if(!verify())throw new AssertionError("MonsterAutoCombatBridgeAudit failed");
    System.out.println("MonsterAutoCombatBridgeAudit PASS");
  }
}
