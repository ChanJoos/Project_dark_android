...
/** Durable F5M quest/reward boundary. Reward claim is persisted before COMPLETED is exposed. */
public final class F5mSaveStore{
 ...
 public static void restoreQuestActive(F5mAdaptedPrologueQuest q){if(active!=null)active.restoreQuest(q);}public static void saveQuestActive(F5mAdaptedPrologueQuest q){if(active!=null)active.saveQuest(q);}public static void restoreRewardsActive(RpgProgressionState r){if(active!=null)active.restoreRewards(r);}public static void saveRewardsActive(RpgProgressionState r){if(active!=null)active.saveRewards(r);}public static boolean rewardClaimedActive(){return active!=null&&active.prefs.getBoolean(K_REWARD_CLAIMED,false);}
 public static boolean commitTurnInActive(F5mAdaptedPrologueQuest q,RpgProgressionState r){if(active==null||q==null||r==null)return false;if(q.state()!=F5mAdaptedPrologueQuest.State.RETURN_READY)return q.state()==F5mAdaptedPrologueQuest.State.COMPLETED&&rewardClaimedActive();int qty=qty(r);if(qty<1)return false;boolean ok=active.prefs.edit().putBoolean(K_REWARD_CLAIMED,true).putInt(K_TOKEN,qty).putString(K_STATE,F5mAdaptedPrologueQuest.State.COMPLETED.name()).putInt(K_COUNT,q.currentCount()).commit();if(ok)q.restore(F5mAdaptedPrologueQuest.State.COMPLETED,q.currentCount());return ok;}
