...
/** PROJECT DARK v0.67 - integrated world/action/NPC/combat/RPG equipment interaction prototype. */
public final class GameView extends View {
  ...
  protected void onDraw(Canvas c){c.drawColor(Color.BLACK);c.save();c.translate(ox,oy);c.scale(scale,scale);drawWorld(c);drawNpcs(c);drawMonsters(c);drawCharacter(c);drawHud(c);drawInventory(c);drawDialogue(c);drawDeath(c);c.restore();}
  ...
  private void bar(Canvas c,float l,float t,float r,float b,int color,float ratio){ratio=Math.max(0,Math.min(1,ratio));p.setColor(0xb51a1714);c.drawRoundRect(new RectF(l,t,r,b),5,5,p);p.setColor(color);c.drawRoundRect(new RectF(l,t,l+(r-l)*ratio,b),5,5,p);}
  ...
  private void drawInventory(Canvas c){if(!inventoryOpen)return;panel(c,620,132,900,410);text(c,"인벤토리",638,156,14);Integer level=state.rpg().normalLevel();text(c,"평민 · Lv "+(level==null?"?":level),638,174,9);List<RpgInventoryPresentation.ItemRow> rows=rpgPresentation.inventoryRows(state.rpg());String selectedId=rpgInteraction.selectedInventoryItemId();if(rows.isEmpty())text(c,"보유 아이템 없음",638,205,11);else{int limit=Math.min(7,rows.size());for(int i=0;i<limit;i++){RpgInventoryPresentation.ItemRow row=rows.get(i);float rowY=203+i*19;if(row.itemId.equals(selectedId)){p.setColor(0x445f4f31);c.drawRoundRect(new RectF(630,rowY-13,892,rowY+5),4,4,p);}String equipped=row.equipped?" [장착]":"";String req=row.requiredLevel==null?"":" · Lv"+row.requiredLevel;text(c,row.name+" x"+row.quantity+equipped,638,rowY,10);text(c,row.itemId+req,760,rowY,8);}if(rows.size()>limit)text(c,"외 "+(rows.size()-limit)+"개",638,203+limit*19,9);}RpgInventoryPresentation.RewardNotice notice=rpgPresentation.latestRewardNotice(state.rpg());if(notice!=null){String rewardState=notice.status==RpgProgressionState.RewardStatus.RESOLVED?"처리됨":"PENDING";text(c,"최근 몬스터 보상: "+rewardState,638,318,9);if(!notice.autoLootedItems.isEmpty())text(c,"자동루팅 아이템 "+notice.autoLootedItems.size()+"종",638,334,9);}if(selectedId!=null){RpgProgressionState.ItemDefinition def=state.rpg().itemDefinitions().get(selectedId);String selectedName=def==null?selectedId:def.name;text(c,"선택: "+selectedName,638,355,9);}else text(c,"아이템을 선택하세요",638,355,9);panel(c,804,342,888,376);text(c,"장착",832,364,11);RpgProgressionState.StatSnapshot stats=rpgInteraction.statSnapshot(state.rpg());text(c,"장비 보정 항목 "+stats.equipment.size()+"개",638,379,9);RpgProgressionState.EquipResult equipResult=rpgInteraction.lastEquipResult();if(equipResult!=null)text(c,equipResultLabel(equipResult),638,397,9);}
  private String equipResultLabel(RpgProgressionState.EquipResult result){switch(result){case EQUIPPED:return "장착 완료";case REQUIREMENT_NOT_MET:return "장착 불가: 요구 레벨 미충족";case REQUIREMENT_PENDING:return "장착 보류: 요구 조건 PENDING";case NOT_EQUIPPABLE:return "장착 불가: 장비 아이템 아님";case UNKNOWN_ITEM:return "장착 불가: 알 수 없는 아이템";case ITEM_NOT_OWNED:default:return "장착 불가: 보유 아이템 아님";}}
  private boolean handleInventoryTouch(float x,float y){if(!inventoryOpen)return false;List<RpgInventoryPresentation.ItemRow> rows=rpgPresentation.inventoryRows(state.rpg());int limit=Math.min(7,rows.size());if(x>=630&&x<=892){for(int i=0;i<limit;i++){float rowY=203+i*19;if(y>=rowY-14&&y<=rowY+6){RpgInventoryPresentation.ItemRow row=rows.get(i);if(rpgInteraction.selectInventoryItem(state.rpg(),row.itemId))showFeedback(row.name+" 선택");return true;}}}if(x>=804&&x<=888&&y>=342&&y<=376){RpgProgressionState.EquipResult result=rpgInteraction.equipSelectedDetailed(state.rpg());showFeedback(equipResultLabel(result));return true;}return x>=620&&x<=900&&y>=132&&y<=410;}

  private void drawDialogue(Canvas c){RuntimeState.Npc dialogNpc=interaction.dialogNpc();if(dialogNpc==null)return;p.setColor(0x66000000);c.drawRect(0,0,W,H,p);panel(c,238,316,722,452);text(c,dialogNpc.name,260,344,14);drawWrappedText(c,dialogNpc.dialogue,260,372,430,12,18);panel(c,598,405,700,438);text(c,"닫기",636,427,11);text(c,"대화 중에는 다른 조작이 잠깁니다 [ADAPTED]",260,435,8);}
  private boolean handleDialogueTouch(float x,float y){if(!interaction.dialogOpen())return false;if(x>=598&&x<=700&&y>=405&&y<=438){interaction.dismissDialog();showFeedback("대화 종료");}return true;}
