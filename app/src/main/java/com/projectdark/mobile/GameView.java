...
public final class GameView extends View {
  ...
  public GameView(Context c){super(c);setKeepScreenOn(true);F5mSaveStore.restoreQuestActive(f5mQuest);F5mSaveStore.restoreRewardsActive(state.rpg());F5mSaveStore.restoreQuest2Active(quest2);quest2.unlockIfPrologueCompleted(f5mQuest);state.applyDerivedGrowth();worldAdapter.snapCameraToPlayer();}
  ...
  private void update(float dt){
    ...
    combat.tick(dt);state.tick(dt);quest2.unlockIfPrologueCompleted(f5mQuest);consumeLedger();consumeRewardNotice();monsterAi.tick(state,dt);
  ...
  private void drawStats(Canvas c){if(!statsOpen)return;modalPanel(c,600,92,936,360);text(c,"CHARACTER STATUS",620,120,13);p.setColor(0xB024262A);c.drawCircle(911,113,15,p);text(c,"×",906,118,14);RpgProgressionState r=state.rpg();mutedText(c,"Lv. "+r.normalLevel()+"    Gold "+r.gold(),620,143,9);text(c,"STR  "+r.str(),624,174,11);text(c,"+",720,174,12);text(c,"INT  "+r.intel(),624,204,11);text(c,"+",720,204,12);text(c,"WIS  "+r.wis(),624,234,11);text(c,"+",720,234,12);text(c,"CON  "+r.con(),624,264,11);text(c,"+",720,264,12);text(c,"DEX  "+r.dex(),624,294,11);text(c,"+",720,294,12);mutedText(c,"POINT  "+r.statPoints(),790,174,10);mutedText(c,"ATK  "+r.physicalAttack(),790,208,10);mutedText(c,"MAX HP  "+r.maxHpGrowth(),790,238,9);mutedText(c,"MAX MP  "+r.maxMpGrowth(),790,266,9);mutedText(c,"레벨업: 기본 성장 + STAT POINT +5",620,334,8);}
  ...
  public boolean onTouchEvent(MotionEvent e){float x=(e.getX()-ox)/scale,y=(e.getY()-oy)/scale;switch(e.getActionMasked()){
    case MotionEvent.ACTION_DOWN:
      if(statsOpen){
        if(circleHit(x,y,UTILITY_X0+UTILITY_STEP,UTILITY_Y0,UTILITY_R+4)||circleHit(x,y,911,113,18)){statsOpen=false;showFeedback("스탯창 닫힘",FeedbackTone.INFO);return true;}
        String st=inside(x,y,700,154,754,186)?"STR":inside(x,y,700,188,754,216)?"INT":inside(x,y,700,218,754,246)?"WIS":inside(x,y,700,248,754,276)?"CON":inside(x,y,700,278,754,306)?"DEX":null;
        ...
        if(!inside(x,y,600,92,936,360)){statsOpen=false;showFeedback("스탯창 닫힘",FeedbackTone.INFO);}return true;
      }
      ...
      if(interaction.dialogOpen()){if(f5mGuideDialogue()&&f5mQuest.state()==F5mAdaptedPrologueQuest.State.COMPLETED&&quest2.state()==GrowthQuest2.State.AVAILABLE&&inside(x,y,500,408,606,442)){if(quest2.accept()){showFeedback("퀘스트 수락 · 성장 훈련",FeedbackTone.INFO);interaction.dismissDialog();}return true;}if(f5mGuideDialogue()&&quest2.state()==GrowthQuest2.State.RETURN_READY&&inside(x,y,500,408,606,442)){if(quest2.turnIn(state.rpg())){state.applyDerivedGrowth();F5mSaveStore.saveRewardsActive(state.rpg());showReward("성장 훈련 완료 · EXP +15000 · Gold +250");interaction.dismissDialog();}return true;}if(f5mGuideDialogue()&&f5mQuest.state()==F5mAdaptedPrologueQuest.State.RETURN_READY&&inside(x,y,500,408,606,442)){F5mTurnInCoordinator.Result tr=F5mQuestUiFlow.confirmTurnIn(f5mQuest,state.rpg());String msg=F5mQuestUiFlow.completionMessage(tr);if(tr==F5mTurnInCoordinator.Result.COMPLETED){showReward(msg);interaction.dismissDialog();worldAdapter.cancelForAction();}else showFeedback(msg,tr==F5mTurnInCoordinator.Result.ALREADY_COMPLETED?FeedbackTone.INFO:FeedbackTone.WARN);return true;}if(f5mGuideDialogue()&&f5mQuest.state()==F5mAdaptedPrologueQuest.State.AVAILABLE&&inside(x,y,500,408,606,442)){F5mAdaptedPrologueQuest.AcceptResult r=f5mQuest.accept();showFeedback(r==F5mAdaptedPrologueQuest.AcceptResult.ACTIVATED?"퀘스트 수락 · 첫 훈련":"퀘스트 상태 유지",FeedbackTone.INFO);return true;}if(f5mGuideDialogue()&&f5mQuest.state()==F5mAdaptedPrologueQuest.State.AVAILABLE&&inside(x,y,620,408,726,442)){f5mQuest.decline();interaction.dismissDialog();worldAdapter.cancelForAction();showFeedback("퀘스트 거절 · 다시 대화할 수 있습니다",FeedbackTone.INFO);return true;}interaction.dismissDialog();worldAdapter.cancelForAction();showFeedback("대화 종료",FeedbackTone.INFO);return true;}
