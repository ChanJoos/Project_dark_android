...
public final class GameView extends View {
  ...
  private void applyToTarget(SkillDef def){RuntimeState.Monster t=combat.target();if(t!=null&&t.alive&&inRange(def.range)){CombatStatPipeline.Channel ch=def.actionClass==SkillDef.ActionClass.MAGIC?CombatStatPipeline.Channel.MAGIC:CombatStatPipeline.Channel.PHYSICAL;int dmg=CombatStatPipeline.resolve(def.damage,ch,state.rpg().finalStats(),FinalStats.neutral(t.maxHp,0)).applied;state.damage(t,dmg);}}
  ...
  private void attack(){worldAdapter.cancelForAction();if(isActing()||!combat.attackReady()||!state.player().alive||!requireTarget())return;AttackDef def=combat.attackDef();if(!inRange(def.range)){beginCombatApproach(CombatController.Intent.ATTACK);return;}snapshotAttackFacingTarget();combat.commitAttack();trigger(actionFor(def.kind));RuntimeState.Monster t=combat.target();if(t!=null&&t.alive){int dmg=CombatStatPipeline.resolve(def.damage,CombatStatPipeline.Channel.PHYSICAL,state.rpg().finalStats(),FinalStats.neutral(t.maxHp,0)).applied;state.damage(t,dmg);}}
