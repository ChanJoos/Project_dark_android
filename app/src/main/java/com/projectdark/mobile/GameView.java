...
public final class GameView extends View {
  ...
  private void drawQuest(Canvas c){F5mAdaptedPrologueQuest.State q=f5mQuest.state();String task;String hint;int progress=f5mQuest.currentCount(),goal=f5mQuest.requiredCount();if(q==F5mAdaptedPrologueQuest.State.AVAILABLE){task="안내인에게 말을 걸어 의뢰 받기";hint="탭하면 안내인에게 이동";progress=0;}else if(q==F5mAdaptedPrologueQuest.State.ACTIVE){task="훈련용 몬스터 처치";hint="탭하면 퀘스트 목표로 이동";}else if(q==F5mAdaptedPrologueQuest.State.RETURN_READY){task="안내인에게 돌아가 보상 받기";hint="탭하면 안내인에게 이동";progress=goal;}else{task="첫 훈련 완료";hint="다음 이야기를 확인하세요";progress=goal;}drawQuestCard("첫 훈련",task,progress,goal,hint,q==F5mAdaptedPrologueQuest.State.RETURN_READY||q==F5mAdaptedPrologueQuest.State.COMPLETED);}
  private void drawQuest2(Canvas c){GrowthQuest2.State q=quest2.state();String task;String hint;int progress=quest2.currentCount(),goal=quest2.requiredCount();if(q==GrowthQuest2.State.AVAILABLE){task="안내인에게 말을 걸어 의뢰 받기";hint="보상  EXP 15,000  ·  Gold 250";progress=0;}else if(q==GrowthQuest2.State.ACTIVE){task="훈련용 몬스터 처치";hint="보상  EXP 15,000  ·  Gold 250";}else{task="안내인에게 돌아가 보상 받기";hint="보상  EXP 15,000  ·  Gold 250";progress=goal;}drawQuestCard("성장 훈련",task,progress,goal,hint,q==GrowthQuest2.State.RETURN_READY);}
  private void drawNextPurpose(Canvas c){drawQuestCard(PostF5mHudPresentation.purposeTitle(f5mQuest.state()),PostF5mHudPresentation.purposeBody(f5mQuest.state()),0,0,"탭하면 퀘스트 안내",false);}
  private void drawQuestCard(String title,String task,int progress,int goal,String hint,boolean ready){
