...
/** PROJECT DARK v0.59 - integrated world/action/NPC/combat runtime prototype. */
public final class GameView extends View {
  private static final float W=960f,H=540f;
  private enum Action { IDLE,WALK,CAST,SWING,THRUST,THROW,PUNCH,SKILL,KICK }
  private enum CombatIntent { NONE,ATTACK,SKILL,KICK }
  ...
  private int dir=0,attackMode=0,defeatedCount=0,playerHitCount=0;
  ...
  private void update(float dt){
    ...
    state.tick(dt);consumeLedger();updateMonsters(dt);
    if(targetMonster!=null&&!targetMonster.alive){targetMonster=null;cancelCombatApproach();}
    if(!state.player().alive){action=Action.IDLE;approachNpc=null;cancelCombatApproach();joy=false;vx=vy=0;knobX=jx;knobY=jy;return;}
    ...
    if(joy&&(vx!=0||vy!=0)){action=Action.WALK;state.tryMove(vx*145*dt,vy*145*dt);walkClock+=dt;approachNpc=null;cancelCombatApproach();approachBlockedClock=0;return;}
    if(approachNpc!=null){updateNpcApproach(dt);return;}
    if(combatApproachMonster!=null&&combatIntent!=CombatIntent.NONE){updateCombatApproach(dt);return;}
    action=Action.IDLE;
  }

  private void consumeLedger(){
    List<CombatLedger.Event> events=state.ledger().snapshot();
    for(CombatLedger.Event e:events){if(e.sequence<=lastLedgerSequence)continue;lastLedgerSequence=e.sequence;switch(e.type){case MONSTER_DEFEATED:defeatedCount++;showFeedback("몬스터 격파 [B]");break;case PLAYER_HIT:playerHitCount++;showFeedback("-"+e.amount+" HP [B]");break;case PLAYER_DEFEATED:showFeedback("행동 불능 [B]");break;case PLAYER_REVIVED:showFeedback("부활 [B]");break;default:break;}}
  }
  ...
  private void updateNpcApproach(float dt){
    ...
    if(approachBlockedClock>2f){approachNpc=null;approachBlockedClock=0;action=Action.IDLE;showFeedback("접근 경로 없음 [B]");}
  }

  private float intentRange(){switch(combatIntent){case SKILL:return SkillDef.SKILL_PROTO.range;case KICK:return SkillDef.KICK_PROTO.range;case ATTACK:return AttackDef.at(attackMode).range;default:return 0f;}}
  private void updateCombatApproach(float dt){
    if(combatApproachMonster==null||!combatApproachMonster.alive){cancelCombatApproach();return;}
    float d=state.distanceTo(combatApproachMonster),range=intentRange();
    if(d<=range){CombatIntent ready=combatIntent;combatApproachMonster=null;combatIntent=CombatIntent.NONE;action=Action.IDLE;if(ready==CombatIntent.ATTACK)attack();else if(ready==CombatIntent.SKILL)skill();else if(ready==CombatIntent.KICK)kick();return;}
    ...
    if(!moved){cancelCombatApproach();action=Action.IDLE;showFeedback("사거리 접근 실패 [B]");}
  }
  private void beginCombatApproach(CombatIntent intent){if(targetMonster==null||!targetMonster.alive)return;combatApproachMonster=targetMonster;combatIntent=intent;showFeedback("타깃 접근 [ADAPTED]");}
  private void cancelCombatApproach(){combatApproachMonster=null;combatIntent=CombatIntent.NONE;}
  ...
  private void trigger(Action a){if(isActing()||!state.player().alive)return;approachNpc=null;dialogNpc=null;cancelCombatApproach();action=a;actionClock=0;}
  ...
  private void applyToTarget(SkillDef def){if(inRange(def.range))state.damage(targetMonster,def.damage);else if(targetMonster==null)showFeedback("타깃 없음");else showFeedback("사거리 밖 [B]");}
  ...
  private void skill(){if(isActing()||skillCooldown>0)return;if(targetMonster!=null&&!inRange(SkillDef.SKILL_PROTO.range)){beginCombatApproach(CombatIntent.SKILL);return;}if(!consume(SkillDef.SKILL_PROTO))return;skillCooldown=SkillDef.SKILL_PROTO.cooldown;trigger(Action.SKILL);applyToTarget(SkillDef.SKILL_PROTO);}
  private void kick(){if(isActing()||kickCooldown>0)return;if(targetMonster!=null&&!inRange(SkillDef.KICK_PROTO.range)){beginCombatApproach(CombatIntent.KICK);return;}kickCooldown=SkillDef.KICK_PROTO.cooldown;trigger(Action.KICK);applyToTarget(SkillDef.KICK_PROTO);}
  ...
  private void attack(){if(isActing()||attackCooldown>0||!state.player().alive)return;AttackDef def=AttackDef.at(attackMode);if(targetMonster!=null&&targetMonster.alive&&!inRange(def.range)){beginCombatApproach(CombatIntent.ATTACK);return;}attackCooldown=def.cooldown;trigger(actionFor(def.kind));if(targetMonster!=null&&targetMonster.alive)state.damage(targetMonster,def.damage);else showFeedback("타깃 없음");}
