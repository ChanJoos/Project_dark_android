...
public final class GameView extends View {
  ...
  private RuntimeState.Monster targetMonster;
  ...
  private void drawWorld(Canvas c){
    ...
    int sw=world.getWidth(),sh=world.getHeight();float da=W/H,sa=sw/(float)sh;Rect s;
    if(sa>da){int uw=Math.round(sh*da),l=(sw-uw)/2;s=new Rect(l,0,l+uw,sh);}else{int uh=Math.round(sw/da),t=(sh-uh)/2;s=new Rect(0,t,sw,t+uh);}
    c.drawBitmap(world,s,new RectF(0,0,W,H),pixel);p.setColor(0x26000000);c.drawRect(0,0,W,H,p);
  }

  private void drawNpcs(Canvas c){
    for(RuntimeState.Npc npc:state.npcs()){
      p.setColor(0x66000000);c.drawOval(new RectF(npc.x-11,npc.y-3,npc.x+11,npc.y+4),p);
      p.setColor(0xffcab58b);c.drawCircle(npc.x,npc.y-39,7,p);p.setColor(0xff6c5946);c.drawRect(npc.x-7,npc.y-31,npc.x+7,npc.y-11,p);
      p.setColor(0xffb58f58);c.drawRect(npc.x-5,npc.y-12,npc.x-1,npc.y-3,p);c.drawRect(npc.x+1,npc.y-12,npc.x+5,npc.y-3,p);
      p.setTextSize(9);p.setColor(0xfff3e1ae);float tw=p.measureText(npc.name);c.drawText(npc.name,npc.x-tw/2,npc.y-52,p);
      if(approachNpc==npc){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2);p.setColor(0xfff0d17a);c.drawCircle(npc.x,npc.y-24,19,p);p.setStyle(Paint.Style.FILL);}
    }
  }
  ...
  private void drawMonsters(Canvas c){
    for(RuntimeState.Monster m:state.monsters()){
      ...
      bar(c,m.x-18,m.y-42,m.x+18,m.y-37,0xffd63442,m.hp/(float)m.maxHp);
      if(targetMonster==m){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2);p.setColor(0xffffd86b);c.drawOval(new RectF(m.x-21,m.y-9,m.x+21,m.y+9),p);p.setStyle(Paint.Style.FILL);}
    }
  }

  private void drawCharacter(Canvas c){
    float px=state.player().x,py=state.player().y,S=2f,x=px,y=py;int f=action==Action.WALK?((int)(walkClock*8f)%4):0;float bob=(f==1||f==3)?-1:0;
    ...
    if(!back){int eye=0xff252020;if(left){rect(c,eye,5,7,6,8);rect(c,eye,9,7,10,8);}else{rect(c,eye,7,7,8,8);rect(c,eye,11,7,12,8);}}
    rect(c,0xff76503b,left?3:11,3,left?4:12,6);
    if(action==Action.SWING||action==Action.THRUST){p.setColor(0xffd7d2c5);p.setStrokeWidth(2);float ex=left?-7:23,ey=action==Action.THRUST?14:6;c.drawLine(left?2:15,14,ex,ey,p);}
    c.restore();
  }

  private void drawActionFx(Canvas c){
    if(!isActing())return;float px=state.player().x,py=state.player().y,q=Math.min(1f,actionClock/duration(action));
    if(action==Action.CAST){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(3);p.setColor(0xaa78b9ff);c.drawCircle(px,py-70,10+18*q,p);c.drawCircle(px,py-70,24-8*q,p);p.setStyle(Paint.Style.FILL);}
    else if(action==Action.THROW){p.setColor(0xffffd76b);float sx=(dir==0||dir==2)?-1:1,sy=(dir<2)?1:-1;c.drawCircle(px+sx*55*q,py-30+sy*35*q,4,p);}
    else if(action==Action.PUNCH||action==Action.KICK||action==Action.SKILL){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(action==Action.SKILL?5:3);p.setColor(action==Action.SKILL?0xaa7fffa8:0xaaffefb0);float sx=(dir==0||dir==2)?-1:1;c.drawArc(new RectF(px+sx*18-18,py-50,px+sx*18+18,py-14),20,130,false,p);p.setStyle(Paint.Style.FILL);}
  }
