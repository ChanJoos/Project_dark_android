...
import com.projectdark.mobile.world.WorldLiveMapLayer;
import com.projectdark.mobile.world.WorldMoveTargetController;
import java.util.List;
import java.util.Map;

/** PROJECT DARK v0.72 - mobile MMORPG HUD/UX shell over existing runtime semantics. */
public final class GameView extends View {
  ...
  private final WorldLiveMapLayer liveMap=new WorldLiveMapLayer(state,W,H);
  ...
  private final Runnable loop=new Runnable(){@Override public void run(){if(!running)return;long n=SystemClock.uptimeMillis();float dt=Math.min(.05f,(n-last)/1000f);last=n;update(dt);camera.follow(state.player().x,state.player().y);liveMap.followPlayer();invalidate();postDelayed(this,16);}};

  public GameView(Context c){super(c);pixel.setFilterBitmap(false);setKeepScreenOn(true);camera.snapTo(state.player().x,state.player().y);liveMap.snapToPlayer();}
  ...
  private void update(float dt){
    ...
    if(move.status==WorldMoveTargetController.Status.MOVING){float bx=state.player().x,by=state.player().y;move=moveTarget.tick(dt);float dx=state.player().x-bx,dy=state.player().y-by;if(dx!=0||dy!=0){setFacingDirection(dx,dy);action=Action.WALK;walkClock+=dt;}else action=Action.IDLE;consumeMoveOutcome(move);return;}
  ...
  private void consumeRewardNotice(){RpgInventoryPresentation.RewardNotice notice=rpgPresentation.latestRewardNotice(state.rpg());if(notice==null||notice.combatSequence<=lastRewardSequence)return;lastRewardSequence=notice.combatSequence;if(notice.status!=RpgProgressionState.RewardStatus.RESOLVED){showReward("보상 확인 필요");return;}if(!notice.autoLootedItems.isEmpty()){Map.Entry<String,Integer> first=null;for(Map.Entry<String,Integer> e:notice.autoLootedItems.entrySet()){first=e;break;}if(first!=null){String name=itemDisplayName(first.getKey());int extra=Math.max(0,notice.autoLootedItems.size()-1);showReward("자동루팅 · "+name+" x"+first.getValue()+(extra>0?" 외 "+extra+"종":""));return;}}if(notice.exp!=null){showReward("보상 처리 · EXP +"+notice.exp);return;}showReward("보상 처리 완료");}
  ...
  private void beginCombatApproach(CombatController.Intent intent){moveTarget.cancelForAction();if(combat.beginApproach(intent))showFeedback("타깃으로 이동",FeedbackTone.INFO);}
  ...
  protected void onDraw(Canvas c){c.drawColor(Color.BLACK);c.save();c.translate(ox,oy);c.scale(scale,scale);liveMap.draw(c);c.save();c.translate(-camera.cameraX(),-camera.cameraY());drawTapMarker(c);drawNpcs(c);drawMonsters(c);drawCharacter(c);c.restore();drawHud(c);drawFeedbackBanners(c);drawInventory(c);drawDialogue(c);drawDeath(c);c.restore();}
  ...
  private void panel(Canvas c,float l,float t,float r,float b){p.setStyle(Paint.Style.FILL);p.setColor(0xB5121418);c.drawRoundRect(new RectF(l,t,r,b),12,12,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.15f);p.setColor(0x88C8A96A);c.drawRoundRect(new RectF(l+.5f,t+.5f,r-.5f,b-.5f),12,12,p);p.setStyle(Paint.Style.FILL);}
  private void text(Canvas c,String s,float x,float y,float size){p.setTextSize(size);p.setColor(0xfff2ead8);c.drawText(s,x,y,p);}private void mutedText(Canvas c,String s,float x,float y,float size){p.setTextSize(size);p.setColor(0xffa9a395);c.drawText(s,x,y,p);}private void bar(Canvas c,float l,float t,float r,float b,int color,float ratio){ratio=Math.max(0,Math.min(1,ratio));p.setColor(0xB318181B);c.drawRoundRect(new RectF(l,t,r,b),5,5,p);p.setColor(color);c.drawRoundRect(new RectF(l,t,l+(r-l)*ratio,b),5,5,p);}
  ...
  private void drawParty(Canvas c){panel(c,16,16,154,101);text(c,"PARTY",28,34,9);String[] n={"조춘찬","수수료좀챙","이루","별빛영혼"};for(int i=0;i<4;i++){float y=50+i*12;text(c,n[i],28,y,8.5f);bar(c,92,y-7,143,y-3,0xffd84555,1f);}}
  private void drawQuest(Canvas c){panel(c,168,16,356,67);text(c,"QUEST",181,34,9);text(c,"밀레스의 첫걸음",181,50,10.5f);mutedText(c,"진행 0 / 4",295,50,8.5f);}
  private void drawTarget(Canvas c){RuntimeState.Monster selected=combat.target();if(selected==null){panel(c,386,16,574,49);mutedText(c,"타깃 없음",453,37,9);return;}panel(c,365,14,596,61);text(c,selected.name,385,34,11);mutedText(c,"TARGET",540,33,8);bar(c,385,42,576,51,0xffd93b4d,selected.hp/(float)selected.maxHp);}
  private void drawMinimap(Canvas c){panel(c,736,16,892,119);text(c,"밀레스",748,34,10);p.setColor(0xB0090B0E);c.drawRoundRect(new RectF(748,42,880,101),7,7,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1);p.setColor(0x557F735E);c.drawRoundRect(new RectF(748,42,880,101),7,7,p);p.setStyle(Paint.Style.FILL);float nx=(state.player().x-WorldDef.MIN_X)/Math.max(1f,WorldDef.MAX_X-WorldDef.MIN_X),ny=(state.player().y-WorldDef.MIN_Y)/Math.max(1f,WorldDef.MAX_Y-WorldDef.MIN_Y);float px=750+Math.max(0,Math.min(1,nx))*128,py=44+Math.max(0,Math.min(1,ny))*55;p.setColor(0xffffdf74);c.drawCircle(px,py,3.2f,p);mutedText(c,"X "+(int)state.player().x+"  Y "+(int)state.player().y,804,113,7.5f);}
  private void drawUtilityRail(Canvas c){String[] labels={"≡","BAG","Q","G","W","⚙"};for(int i=0;i<labels.length;i++){float cy=34+i*46;boolean active=i==1&&inventoryOpen;p.setColor(active?0xD05F4D2E:0xB0131519);c.drawCircle(926,cy,17,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(active?2f:1.1f);p.setColor(active?0xffffd77a:0x99bfa46b);c.drawCircle(926,cy,17,p);p.setStyle(Paint.Style.FILL);p.setTextSize(labels[i].length()>1?7:12);p.setColor(active?0xffffebbd:0xffe7ddc8);float tw=p.measureText(labels[i]);c.drawText(labels[i],926-tw/2,cy+3,p);}}
  private void drawChat(Canvas c){RuntimeMetrics metrics=state.metrics();panel(c,16,310,278,380);mutedText(c,"[일반]  밀레스",28,329,8.5f);text(c,"격파 "+metrics.monsterDefeats()+"   피격 "+metrics.playerHits()+"   DMG "+metrics.damageDealt(),28,347,8.5f);mutedText(c,"NPC · 전투 · 보상 알림",28,365,8);}
  private void drawJoystick(Canvas c){float active=joy?1f:0f;p.setColor(joy?0x553D4652:0x332B3038);c.drawCircle(jx,jy,jr,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(joy?2.5f:1.5f);p.setColor(joy?0xCCDBC386:0x777F7562);c.drawCircle(jx,jy,jr,p);p.setStrokeWidth(1);p.setColor(0x557F7562);c.drawCircle(jx,jy,38,p);p.setStyle(Paint.Style.FILL);p.setColor(joy?0xD5C7A86B:0xA47A6C54);c.drawCircle(knobX,knobY,23+2*active,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.5f);p.setColor(0xDDEAD9B0);c.drawCircle(knobX,knobY,23+2*active,p);p.setStyle(Paint.Style.FILL);}
  private void drawPlayerStatus(Canvas c){panel(c,336,468,610,529);Integer level=state.rpg().normalLevel();text(c,"Lv "+(level==null?"?":level),349,487,11);mutedText(c,"평민",349,503,8.5f);bar(c,397,477,593,487,0xffd83c4d,state.player().hp/(float)state.player().maxHp);bar(c,397,493,593,503,0xff337fd6,state.player().mp/(float)state.player().maxMp);bar(c,397,511,593,518,0xff55a95f,.35f);mutedText(c,"HP",374,485,7.5f);mutedText(c,"MP",374,501,7.5f);mutedText(c,"EXP",370,518,7.5f);}
  private void drawCombatCluster(Canvas c){boolean target=combat.hasUsableTarget();boolean atkEnabled=target&&combat.attackReady()&&state.player().alive;boolean magEnabled=target&&combat.castReady()&&state.player().alive&&state.player().mp>=SkillDef.CAST_PROTO.mpCost;boolean skillEnabled=target&&combat.skillReady()&&state.player().alive&&state.player().mp>=SkillDef.SKILL_PROTO.mpCost;boolean kickEnabled=target&&combat.kickReady()&&state.player().alive;actionButton(c,888,466,40,"ATK",atkEnabled,attacking(),"ATK".equals(pressedControl),combat.attackCooldown(),combat.attackDef().cooldown,true);actionButton(c,826,413,27,"SKILL",skillEnabled,action==Action.SKILL,"SKILL".equals(pressedControl),combat.skillCooldown(),SkillDef.SKILL_PROTO.cooldown,false);actionButton(c,808,478,27,"MAG",magEnabled,action==Action.CAST,"MAG".equals(pressedControl),combat.castCooldown(),SkillDef.CAST_PROTO.cooldown,false);actionButton(c,762,430,24,"KICK",kickEnabled,action==Action.KICK,"KICK".equals(pressedControl),combat.kickCooldown(),SkillDef.KICK_PROTO.cooldown,false);actionButton(c,748,492,22,"MODE",true,false,"MODE".equals(pressedControl),0,0,false);actionButton(c,927,515,20,"AUTO",false,false,"AUTO".equals(pressedControl),0,0,false);mutedText(c,combat.attackDef().label,846,526,7.5f);}
  private void actionButton(Canvas c,float x,float y,float r,String label,boolean enabled,boolean selected,boolean pressed,float cooldown,float total,boolean primary){float rr=pressed?r-2:r;p.setColor(enabled?(primary?0xE05B3A28:0xD022252A):0xB0101114);c.drawCircle(x,y,rr,p);if(selected){p.setColor(0x44FFE19A);c.drawCircle(x,y,rr+5,p);}p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(primary?2.6f:1.7f);p.setColor(enabled?(selected?0xffffe69d:0xffc8aa6c):0x665f5a50);c.drawCircle(x,y,rr,p);p.setStyle(Paint.Style.FILL);p.setTextSize(primary?11:(label.length()>4?7:8.5f));p.setColor(enabled?0xfffff0cf:0xff74716a);float tw=p.measureText(label);c.drawText(label,x-tw/2,y+3,p);cooldown(c,x,y,rr,cooldown,total);if(!enabled&&"AUTO".equals(label)){p.setTextSize(7);p.setColor(0xff8f8a80);c.drawText("LOCK",x-9,y+13,p);}}
  ...
  private void drawFeedbackBanners(Canvas c){if(rewardClock>0){float a=Math.min(1f,rewardClock/.35f);p.setColor((int)(0xD0*a)<<24|0x2B2517);c.drawRoundRect(new RectF(344,76,616,110),17,17,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.5f);p.setColor(0xCCDBB768);c.drawRoundRect(new RectF(344,76,616,110),17,17,p);p.setStyle(Paint.Style.FILL);p.setTextSize(10.5f);p.setColor(0xffffe5a3);float tw=p.measureText(rewardBanner);c.drawText(rewardBanner,480-tw/2,98,p);}if(feedbackClock>0){int fill=feedbackTone==FeedbackTone.WARN?0xD04A2226:feedbackTone==FeedbackTone.REWARD?0xD03C321E:0xCE161A20;p.setColor(fill);float tw=Math.min(360,Math.max(150,p.measureText(feedback)+44));c.drawRoundRect(new RectF(480-tw/2,116,480+tw/2,145),14,14,p);p.setTextSize(9.5f);p.setColor(feedbackTone==FeedbackTone.WARN?0xffffb4ad:0xffeee5d3);float ft=p.measureText(feedback);c.drawText(feedback,480-ft/2,135,p);}}

  private void drawInventory(Canvas c){if(!inventoryOpen)return;p.setColor(0x55000000);c.drawRect(0,0,W,H,p);panel(c,588,92,904,444);text(c,"INVENTORY",608,118,13);Integer level=state.rpg().normalLevel();mutedText(c,"평민 · Lv "+(level==null?"?":level),608,135,8.5f);p.setColor(0xB024262A);c.drawCircle(879,113,15,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.2f);p.setColor(0x99C8A96A);c.drawCircle(879,113,15,p);p.setStyle(Paint.Style.FILL);text(c,"×",874,118,14);List<RpgInventoryPresentation.ItemRow> rows=rpgPresentation.inventoryRows(state.rpg());String selectedId=rpgInteraction.selectedInventoryItemId();if(rows.isEmpty())mutedText(c,"보유 아이템 없음",608,171,10);else{int limit=Math.min(7,rows.size());for(int i=0;i<limit;i++){RpgInventoryPresentation.ItemRow row=rows.get(i);float y=169+i*27;boolean selected=row.itemId.equals(selectedId);p.setColor(selected?0x665D4A28:0x5517191D);c.drawRoundRect(new RectF(602,y-16,890,y+6),7,7,p);if(selected){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.3f);p.setColor(0xBBD2AF67);c.drawRoundRect(new RectF(602,y-16,890,y+6),7,7,p);p.setStyle(Paint.Style.FILL);}text(c,row.name+"  x"+row.quantity+(row.equipped?"  [장착]":""),614,y-2,9.5f);mutedText(c,row.itemId,790,y-2,7.5f);}if(rows.size()>limit)mutedText(c,"외 "+(rows.size()-limit)+"개",610,169+limit*27,8);}RpgInventoryPresentation.ItemRow selectedRow=null;if(selectedId!=null)for(RpgInventoryPresentation.ItemRow row:rows)if(selectedId.equals(row.itemId)){selectedRow=row;break;}p.setColor(0x5517191D);c.drawRoundRect(new RectF(602,356,890,426),8,8,p);if(selectedRow!=null){text(c,selectedRow.name,614,375,10.5f);mutedText(c,rpgPresentation.requirementLabel(selectedRow),614,391,8.5f);String element=rpgPresentation.elementLabel(selectedRow);if(!element.isEmpty())mutedText(c,element,614,406,8);}else mutedText(c,"아이템을 선택하세요",614,386,9);p.setColor(0xD05B4727);c.drawRoundRect(new RectF(810,389,880,419),9,9,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.2f);p.setColor(0xCCD3B16D);c.drawRoundRect(new RectF(810,389,880,419),9,9,p);p.setStyle(Paint.Style.FILL);text(c,"장착",833,409,10);RpgInventoryPresentation.RewardNotice notice=rpgPresentation.latestRewardNotice(state.rpg());if(notice!=null)mutedText(c,"최근 보상 · "+(notice.status==RpgProgressionState.RewardStatus.RESOLVED?"처리됨":"확인 필요"),608,438,7.5f);}
  ...
  private void drawDialogue(Canvas c){RuntimeState.Npc dialogNpc=interaction.dialogNpc();if(dialogNpc==null)return;p.setColor(0x72000000);c.drawRect(0,0,W,H,p);panel(c,190,326,770,462);p.setColor(0xD05A472A);c.drawRoundRect(new RectF(210,342,338,371),8,8,p);text(c,dialogNpc.name,226,361,12);drawWrappedText(c,dialogNpc.dialogue,218,394,510,11.5f,18);mutedText(c,"화면을 터치하면 닫기",616,446,8.5f);p.setColor(0xB024262A);c.drawCircle(744,347,14,p);text(c,"×",739,352,14);}
  private void drawDeath(Canvas c){if(state.player().alive)return;p.setColor(0xB6000000);c.drawRect(0,0,W,H,p);panel(c,330,205,630,335);text(c,"행동 불능",428,242,19);mutedText(c,"프로토타입 부활",432,270,10);p.setColor(0xD05B4727);c.drawRoundRect(new RectF(405,286,555,320),10,10,p);text(c,"화면 중앙 터치",433,308,11);}
  ...
  private boolean isHudSurface(float x,float y){
    if((x>=16&&x<=154&&y>=16&&y<=101)||(x>=168&&x<=356&&y>=16&&y<=67)||(x>=365&&x<=596&&y>=14&&y<=61)||(x>=736&&x<=892&&y>=16&&y<=119)||(x>=16&&x<=278&&y>=310&&y<=380)||(x>=336&&x<=610&&y>=468&&y<=529))return true;
    for(int i=0;i<6;i++)if(dist(x,y,926,34+i*46)<=19f)return true;
    return false;
  }
  private void requestGroundMove(float x,float y){WorldCameraTransform.Point wp=liveMap.screenToWorld(x,y);WorldMoveTargetController.Snapshot move=moveTarget.requestGroundMove(wp.x,wp.y);lastMoveRequestId=move.requestId;lastMoveStatus=WorldMoveTargetController.Status.IDLE;if(move.status==WorldMoveTargetController.Status.MOVING||move.status==WorldMoveTargetController.Status.REACHED){tapMarkerX=wp.x;tapMarkerY=wp.y;tapMarkerClock=.7f;showFeedback(move.replacedRequestId>0?"이동 목표 변경":"이동 시작",FeedbackTone.INFO);}consumeMoveOutcome(move);}

  public boolean onTouchEvent(MotionEvent e){float x=(e.getX()-ox)/scale,y=(e.getY()-oy)/scale;switch(e.getActionMasked()){
    case MotionEvent.ACTION_DOWN:
      if(!state.player().alive){if(dist(x,y,480,270)<=180){state.revivePlayer();combat.clearTarget();interaction.cancel();moveTarget.cancel();camera.snapTo(state.player().x,state.player().y);liveMap.snapToPlayer();action=Action.IDLE;}return true;}
      if(interaction.dialogOpen()){interaction.dismissDialog();moveTarget.cancelForAction();showFeedback("대화 종료",FeedbackTone.INFO);return true;}
      if(x>=906&&x<=948&&y>=55&&y<=101){inventoryOpen=!inventoryOpen;moveTarget.cancelForAction();showFeedback(inventoryOpen?"인벤토리 열림":"인벤토리 닫힘",FeedbackTone.INFO);return true;}
      if(handleInventoryTouch(x,y)){moveTarget.cancelForAction();return true;}
      if(inventoryOpen){inventoryOpen=false;moveTarget.cancelForAction();showFeedback("인벤토리 닫힘",FeedbackTone.INFO);return true;}
      if(dist(x,y,jx,jy)<=jr*1.5f){joy=true;pressedControl="JOY";moveTarget.cancelForDirectInput();interaction.cancelApproach();combat.cancelApproach();stick(x,y);return true;}
      if(dist(x,y,808,478)<=34){pressedControl="MAG";cast();return true;}
      if(dist(x,y,748,492)<=29){pressedControl="MODE";moveTarget.cancelForAction();combat.cycleAttackMode();showFeedback(combat.attackDef().label,FeedbackTone.INFO);return true;}
      if(dist(x,y,826,413)<=34){pressedControl="SKILL";skill();return true;}
      if(dist(x,y,762,430)<=31){pressedControl="KICK";kick();return true;}
      if(dist(x,y,888,466)<=48){pressedControl="ATK";attack();return true;}
      if(dist(x,y,927,515)<=27){pressedControl="AUTO";showFeedback("AUTO 준비 중",FeedbackTone.WARN);return true;}
      if(isHudSurface(x,y))return true;
      WorldCameraTransform.Point wp=liveMap.screenToWorld(x,y);RuntimeState.Npc npc=state.hitNpc(wp.x,wp.y,34f);if(npc!=null){moveTarget.cancelForAction();combat.cancelApproach();interaction.request(state,npc);showFeedback("NPC 접근 · "+npc.name,FeedbackTone.INFO);return true;}RuntimeState.Monster monster=state.hitMonster(wp.x,wp.y,34f);if(monster!=null){moveTarget.cancelForAction();combat.selectTarget(monster);interaction.cancelApproach();showFeedback("타깃 선택 · "+monster.name,FeedbackTone.INFO);return true;}requestGroundMove(x,y);return true;
