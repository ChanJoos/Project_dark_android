...
/** PROJECT DARK v0.60 - integrated world/action/NPC/combat runtime prototype. */
public final class GameView extends View {
  ...
  private void update(float dt){
    ...
    if(!state.player().alive){action=Action.IDLE;approachNpc=null;combat.cancelApproach();joy=false;vx=vy=0;knobX=jx;knobY=jy;return;}
    ...
    if(joy&&(vx!=0||vy!=0)){action=Action.WALK;state.tryMove(vx*145*dt,vy*145*dt);walkClock+=dt;approachNpc=null;combat.cancelApproach();approachBlockedClock=0;return;}
  ...
  private void trigger(Action a){if(isActing()||!state.player().alive)return;approachNpc=null;dialogNpc=null;combat.cancelApproach();action=a;actionClock=0;}
  ...
  public boolean onTouchEvent(MotionEvent e){float x=(e.getX()-ox)/scale,y=(e.getY()-oy)/scale;switch(e.getActionMasked()){case MotionEvent.ACTION_DOWN:if(!state.player().alive){if(dist(x,y,480,270)<=180){state.revivePlayer();combat.clearTarget();action=Action.IDLE;}return true;}if(dialogNpc!=null){dialogNpc=null;return true;}RuntimeState.Npc npc=state.hitNpc(x,y,34f);if(npc!=null){combat.cancelApproach();if(state.distanceTo(npc)<=56f)dialogNpc=npc;else{approachNpc=npc;approachBlockedClock=0;}return true;}RuntimeState.Monster monster=state.hitMonster(x,y,34f);if(monster!=null){combat.selectTarget(monster);approachNpc=null;return true;}if(dist(x,y,jx,jy)<=jr*1.5f){joy=true;approachNpc=null;combat.cancelApproach();stick(x,y);return true;}if(dist(x,y,698,466)<=25){cast();return true;}if(dist(x,y,746,466)<=25){combat.cycleAttackMode();showFeedback(combat.attackDef().label+" [B]");return true;}if(dist(x,y,794,466)<=25){skill();return true;}if(dist(x,y,842,466)<=25){kick();return true;}if(dist(x,y,895,478)<=42){attack();return true;}return true;case MotionEvent.ACTION_MOVE:if(joy)stick(x,y);return true;case MotionEvent.ACTION_UP:case MotionEvent.ACTION_CANCEL:joy=false;knobX=jx;knobY=jy;vx=vy=0;return true;}return true;}
