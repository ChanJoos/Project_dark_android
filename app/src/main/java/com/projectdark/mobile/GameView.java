...
public final class GameView extends View {
  ...
  private void drawEquipment(Canvas c){if(!equipmentOpen)return;modalPanel(c,548,72,936,494);text(c,"CHARACTER EQUIPMENT",570,99,13);p.setColor(0xB024262A);c.drawCircle(911,91,15,p);text(c,"×",906,96,14);Map<String,String> eq=state.rpg().equipment();String[] slots={"무기","갑옷","모자","방패","장갑","각반","신발","귀걸이","반지","목걸이","벨트"};int y=132;for(String slot:slots){String id=eq.get(slot);RpgProgressionState.ItemDefinition d=id==null?null:state.rpg().itemDefinitions().get(id);p.setColor(0x5517191D);c.drawRoundRect(new RectF(570,y-16,914,y+7),7,7,p);text(c,slot,584,y,9);fittedText(c,d==null?"-":d.name,660,y,238,9);y+=30;}mutedText(c,"장착/해제는 BAG에서 아이템을 선택하세요",570,482,8);}
