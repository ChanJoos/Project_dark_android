...
/** PROJECT DARK v0.68 - camera-aware tap movement + integrated mobile RPG runtime. */
public final class GameView extends View {
  ...
  private final WorldRuntimeBridge worldBridge=new WorldRuntimeBridge(state,W,H);
  ...
  private int dir=0;
  private float walkClock=0,actionClock=0,tapMarkerClock=0,tapMarkerWorldX=0,tapMarkerWorldY=0;
  ...
  private void update(float dt){
    ...
    if(!state.player().alive){action=Action.IDLE;interaction.cancel();combat.cancelApproach();worldBridge.cancel();joy=false;vx=vy=0;knobX=jx;knobY=jy;return;}
    if(isActing()){actionClock+=dt;if(actionClock>=duration(action)){actionClock=0;action=(joy&&(vx!=0||vy!=0))?Action.WALK:Action.IDLE;}worldBridge.followCamera();return;}
    if(joy&&(vx!=0||vy!=0)){action=Action.WALK;state.tryMove(vx*145*dt,vy*145*dt);walkClock+=dt;interaction.cancelApproach();combat.cancelApproach();worldBridge.followCamera();return;}
    if(interaction.approaching()){updateNpcInteraction(dt);worldBridge.followCamera();return;}
    if(combat.approaching()){updateCombatApproach(dt);worldBridge.followCamera();return;}
    WorldMoveTargetController.Snapshot move=worldBridge.snapshot();
    if(move.status==WorldMoveTargetController.Status.MOVING){
      float bx=state.player().x,by=state.player().y;
      move=worldBridge.tick(dt);
      float dx=state.player().x-bx,dy=state.player().y-by;
      if(dx!=0||dy!=0){setFacingDirection(dx,dy);action=Action.WALK;walkClock+=dt;}
      else action=Action.IDLE;
      consumeMoveOutcome(move);
      return;
    }
    worldBridge.followCamera();consumeMoveOutcome(move);action=Action.IDLE;
  }
  ...
  private void beginCombatApproach(CombatController.Intent intent){worldBridge.cancelForAction();if(combat.beginApproach(intent))showFeedback("타깃 접근 [ADAPTED]");}
  ...
  private void trigger(Action a){if(isActing()||!state.player().alive)return;worldBridge.cancelForAction();interaction.cancel();combat.cancelApproach();action=a;actionClock=0;}
  ...
  private void cast(){worldBridge.cancelForAction();if(isActing()||!combat.castReady()||!requireTarget())return;if(!inRange(SkillDef.CAST_PROTO.range)){beginCombatApproach(CombatController.Intent.CAST);return;}if(!consume(SkillDef.CAST_PROTO))return;faceTarget();combat.commitCast();trigger(Action.CAST);applyToTarget(SkillDef.CAST_PROTO);}
  private void skill(){worldBridge.cancelForAction();if(isActing()||!combat.skillReady()||!requireTarget())return;if(!inRange(SkillDef.SKILL_PROTO.range)){beginCombatApproach(CombatController.Intent.SKILL);return;}if(!consume(SkillDef.SKILL_PROTO))return;faceTarget();combat.commitSkill();trigger(Action.SKILL);applyToTarget(SkillDef.SKILL_PROTO);}
  private void kick(){worldBridge.cancelForAction();if(isActing()||!combat.kickReady()||!requireTarget())return;if(!inRange(SkillDef.KICK_PROTO.range)){beginCombatApproach(CombatController.Intent.KICK);return;}faceTarget();combat.commitKick();trigger(Action.KICK);applyToTarget(SkillDef.KICK_PROTO);}
  ...
  private void attack(){worldBridge.cancelForAction();if(isActing()||!combat.attackReady()||!state.player().alive||!requireTarget())return;AttackDef def=combat.attackDef();if(!inRange(def.range)){beginCombatApproach(CombatController.Intent.ATTACK);return;}faceTarget();combat.commitAttack();trigger(actionFor(def.kind));RuntimeState.Monster t=combat.target();if(t!=null&&t.alive)state.damage(t,def.damage);}

  protected void onSizeChanged(int w,int h,int ow,int oh){scale=Math.min(w/W,h/H);ox=(w-W*scale)/2;oy=(h-H*scale)/2;}
  protected void onDraw(Canvas c){c.drawColor(Color.BLACK);c.save();c.translate(ox,oy);c.scale(scale,scale);drawWorld(c);drawTapMarker(c);drawNpcs(c);drawMonsters(c);drawCharacter(c);drawHud(c);drawInventory(c);drawDialogue(c);drawDeath(c);c.restore();}

  private void drawWorld(Canvas c){
    if(world==null){p.setColor(0xff090909);c.drawRect(0,0,W,H,p);return;}
    int sw=world.getWidth(),sh=world.getHeight();
    WorldCameraTransform.Point tl=worldBridge.screenToWorld(0,0),br=worldBridge.screenToWorld(W,H);
    float ww=Math.max(1f,RuntimeState.WORLD_MAX_X-RuntimeState.WORLD_MIN_X),wh=Math.max(1f,RuntimeState.WORLD_MAX_Y-RuntimeState.WORLD_MIN_Y);
    int l=Math.max(0,Math.min(sw-1,Math.round((tl.x-RuntimeState.WORLD_MIN_X)/ww*sw)));
    int t=Math.max(0,Math.min(sh-1,Math.round((tl.y-RuntimeState.WORLD_MIN_Y)/wh*sh)));
    int r=Math.max(l+1,Math.min(sw,Math.round((br.x-RuntimeState.WORLD_MIN_X)/ww*sw)));
    int b=Math.max(t+1,Math.min(sh,Math.round((br.y-RuntimeState.WORLD_MIN_Y)/wh*sh)));
    c.drawBitmap(world,new Rect(l,t,r,b),new RectF(0,0,W,H),pixel);p.setColor(0x26000000);c.drawRect(0,0,W,H,p);
  }

  private void drawTapMarker(Canvas c){if(tapMarkerClock<=0)return;WorldCameraTransform.Point s=worldBridge.worldToScreen(tapMarkerWorldX,tapMarkerWorldY);float q=Math.max(0f,Math.min(1f,tapMarkerClock/.55f));p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2f);p.setColor(((int)(0xcc*q)<<24)|0x00ffd86b);c.drawCircle(s.x,s.y,10f+(1f-q)*7f,p);c.drawLine(s.x-5,s.y,s.x+5,s.y,p);c.drawLine(s.x,s.y-5,s.x,s.y+5,p);p.setStyle(Paint.Style.FILL);}

  private void drawNpcs(Canvas c){for(RuntimeState.Npc n:state.npcs()){WorldCameraTransform.Point s=worldBridge.worldToScreen(n.x,n.y);float x=s.x,y=s.y;p.setColor(0x66000000);c.drawOval(new RectF(x-11,y-3,x+11,y+4),p);p.setColor(0xffcab58b);c.drawCircle(x,y-39,7,p);p.setColor(0xff6c5946);c.drawRect(x-7,y-31,x+7,y-11,p);p.setColor(0xffb58f58);c.drawRect(x-5,y-12,x-1,y-3,p);c.drawRect(x+1,y-12,x+5,y-3,p);p.setTextSize(9);p.setColor(0xfff3e1ae);float tw=p.measureText(n.name);c.drawText(n.name,x-tw/2,y-52,p);if(interaction.approachNpc()==n){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2);p.setColor(0xfff0d17a);c.drawCircle(x,y-24,19,p);p.setStyle(Paint.Style.FILL);}}}

  private void drawMonsters(Canvas c){RuntimeState.Monster selected=combat.target();for(RuntimeState.Monster m:state.monsters()){if(!m.alive)continue;WorldCameraTransform.Point s=worldBridge.worldToScreen(m.x,m.y);float x=s.x,y=s.y;p.setColor(0x66000000);c.drawOval(new RectF(x-15,y-4,x+15,y+5),p);p.setColor(m.hitFlash>0?0xffd9e8d7:0xff6a7c69);c.drawOval(new RectF(x-13,y-31,x+13,y-5),p);p.setColor(0xffd8c27b);c.drawCircle(x-5,y-20,2,p);c.drawCircle(x+5,y-20,2,p);if(m.attackPrimed){float q=1f-Math.min(1f,m.attackWindup/.24f);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2+2*q);p.setColor(0xaaff7755);c.drawCircle(x,y-19,18+8*q,p);p.setStyle(Paint.Style.FILL);}bar(c,x-18,y-42,x+18,y-37,0xffd63442,m.hp/(float)m.maxHp);if(m.damagePopupClock>0){p.setTextSize(12);p.setColor(0xffffdc72);String d="-"+m.lastDamage;float tw=p.measureText(d);c.drawText(d,x-tw/2,y-50-(.65f-m.damagePopupClock)*20,p);}if(selected==m){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2);p.setColor(0xffffd86b);c.drawOval(new RectF(x-21,y-9,x+21,y+9),p);p.setStyle(Paint.Style.FILL);}}}
  ...
  private void drawCharacter(Canvas c){CharacterRenderer.State presentation=characterState();CharacterVisualBinding visuals=CharacterVisualBinding.from(state.rpg());float stateDuration=isActing()?duration(action):1f;WorldCameraTransform.Point s=worldBridge.worldToScreen(state.player().x,state.player().y);characterRenderer.draw(c,new CharacterRenderer.Pose(s.x,s.y,characterDirection(),presentation,walkClock,actionClock,stateDuration,state.player().hitFlash>0,visuals.equipmentVisualRef(),visuals.weaponVisualRef(),CharacterRenderer.ASSET_STATUS,characterEffectFamily()));}
  ...
  private boolean uiOwnedTouch(float x,float y){if(y<=130)return true;if(x>=912)return true;if(inventoryOpen&&x>=606&&x<=904&&y>=124&&y<=430)return true;if(y>=430)return true;if(x<=285&&y>=290)return true;return false;}

  public boolean onTouchEvent(MotionEvent e){
    float x=(e.getX()-ox)/scale,y=(e.getY()-oy)/scale;
    switch(e.getActionMasked()){
      case MotionEvent.ACTION_DOWN:
        if(!state.player().alive){if(dist(x,y,480,270)<=180){state.revivePlayer();combat.clearTarget();interaction.cancel();worldBridge.cancel();worldBridge.snapCameraToPlayer();action=Action.IDLE;}return true;}
        if(interaction.dialogOpen()){interaction.dismissDialog();worldBridge.cancel();return true;}
        if(x>=912&&x<=948&&y>=70&&y<=108){inventoryOpen=!inventoryOpen;worldBridge.cancelForDirectInput();return true;}
        if(handleInventoryTouch(x,y)){worldBridge.cancelForDirectInput();return true;}
        if(dist(x,y,jx,jy)<=jr*1.5f){joy=true;worldBridge.cancelForDirectInput();interaction.cancelApproach();combat.cancelApproach();stick(x,y);return true;}
        if(dist(x,y,698,466)<=25){cast();return true;}
        if(dist(x,y,746,466)<=25){worldBridge.cancelForAction();combat.cycleAttackMode();showFeedback(combat.attackDef().label+" [B]");return true;}
        if(dist(x,y,794,466)<=25){skill();return true;}
        if(dist(x,y,842,466)<=25){kick();return true;}
        if(dist(x,y,895,478)<=42){attack();return true;}
        if(uiOwnedTouch(x,y))return true;
        WorldCameraTransform.Point wp=worldBridge.screenToWorld(x,y);
        RuntimeState.Npc npc=state.hitNpc(wp.x,wp.y,34f);
        if(npc!=null){worldBridge.cancel();combat.cancelApproach();interaction.request(state,npc);return true;}
        RuntimeState.Monster monster=state.hitMonster(wp.x,wp.y,34f);
        if(monster!=null){worldBridge.cancelForAction();combat.selectTarget(monster);interaction.cancelApproach();showFeedback("타깃 선택 · "+monster.name);return true;}
        interaction.cancelApproach();combat.cancelApproach();
        WorldMoveTargetController.Snapshot requested=worldBridge.requestGroundMove(x,y);
        lastMoveRequestId=requested.requestId;lastMoveStatus=WorldMoveTargetController.Status.IDLE;
        if(requested.status==WorldMoveTargetController.Status.MOVING||requested.status==WorldMoveTargetController.Status.REACHED){tapMarkerWorldX=requested.targetX;tapMarkerWorldY=requested.targetY;tapMarkerClock=.55f;showFeedback(requested.status==WorldMoveTargetController.Status.REACHED?"이미 도착":"이동");}
        else showFeedback("이동 불가");
        consumeMoveOutcome(requested);
        return true;
      case MotionEvent.ACTION_MOVE:if(joy)stick(x,y);return true;
      case MotionEvent.ACTION_UP:case MotionEvent.ACTION_CANCEL:joy=false;knobX=jx;knobY=jy;vx=vy=0;return true;
    }
    return true;
  }
