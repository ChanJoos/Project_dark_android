...
public final class GameView extends View {
  ...
  private void snapshotAttackFacingTarget(){RuntimeState.Monster t=combat.target();if(t==null||!t.alive)return; CharacterRenderer.Direction canonical=combat.attackFacing(state); if(canonical!=null){playerFacing.beginAttack(canonical);playerFacing.setLocomotion(canonical);return;} playerFacing.beginAttack(t.x-state.player().x,t.y-state.player().y);playerFacing.setLocomotion(playerFacing.attack());}
