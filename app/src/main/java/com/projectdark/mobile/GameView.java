...
public final class GameView extends View {
  ...
  private final ReagentItemVisualRegistry reagentVisuals=new ReagentItemVisualRegistry(null);
  private boolean reagentShopOpen=false;
  ...
  private void update(float dt){
    ...
    if(worldAdapter.atReagentShopExit()){worldAdapter.leaveReagentShop();showFeedback("밀레스",FeedbackTone.INFO);return;}
    if(joy&&(vx!=0||vy!=0)){
  ...
  private void consumeMoveOutcome(WorldMoveTargetController.Snapshot move){
    ...
    if(move.status==WorldMoveTargetController.Status.REACHED){
      if(!worldAdapter.inReagentShop()&&move.kind==WorldMoveTargetController.RequestKind.GROUND&&reagentShopEntranceReached()){worldAdapter.enterReagentShop();combat.clearTarget();interaction.cancel();showFeedback("시약상점",FeedbackTone.INFO);return;}
  ...
  protected void onDraw(Canvas c){c.drawColor(Color.BLACK);c.save();c.translate(ox,oy);c.scale(scale,scale);drawWorld(c);WorldCameraTransform activeCamera=worldAdapter.camera();c.save();c.translate(-activeCamera.cameraX(),-activeCamera.cameraY());drawTapMarker(c);if(!worldAdapter.inReagentShop()){drawNpcs(c);drawMonsters(c);}drawCharacter(c);c.restore();drawHud(c);drawFeedbackBanners(c);if(worldAdapter.inReagentShop())drawReagentShop(c);drawInventory(c);drawStats(c);drawDialogue(c);drawDeath(c);c.restore();}

  private void drawWorld(Canvas c){p.setColor(0xff101511);c.drawRect(0,0,W,H,p);if(worldAdapter.inReagentShop()){reagentShopRenderer.draw(c,worldAdapter);WorldCameraTransform.Point m=worldAdapter.worldToScreen(ReagentShopMapDef.MERLIN_X,ReagentShopMapDef.MERLIN_Y);c.save();c.translate(m.x,m.y);c.scale(.78f,.78f);characterRenderer.draw(c,new CharacterRenderer.Pose(0,0,CharacterRenderer.Direction.SE,CharacterRenderer.State.IDLE,0,0,1,false,"mu0000118,mh259",null,CharacterRenderer.ASSET_STATUS,CharacterRenderer.EffectFamily.NONE));c.restore();text(c,"멀린",m.x-13,m.y-42,9);}else mapRenderer.draw(c,worldAdapter);}
  private void drawReagentShop(Canvas c){if(!reagentShopOpen)return;p.setColor(0x5c000000);c.drawRect(0,0,W,H,p);modalPanel(c,430,74,918,454);text(c,"멀린의 시약상점",452,105,14);text(c,"×",887,102,14);String[] n={"코마디움","디베노뭄","쿠라눔","밀레스리콜"};String[] id={RpgProgressionState.REAGENT_KOMADIUM_ITEM_ID,RpgProgressionState.REAGENT_DIBENOMUM_ITEM_ID,RpgProgressionState.REAGENT_CURANUM_ITEM_ID,RpgProgressionState.RECALL_MILLES_ITEM_ID};for(int i=0;i<4;i++){float t=128+i*70;p.setColor(0xD0191411);c.drawRect(456,t,892,t+58,p);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.2f);p.setColor(0xFF765335);c.drawRect(456.5f,t+.5f,891.5f,t+57.5f,p);p.setStyle(Paint.Style.FILL);Bitmap b=reagentVisuals.get(id[i]);if(b!=null){float sc=Math.min(28f/b.getWidth(),28f/b.getHeight()),dw=b.getWidth()*sc,dh=b.getHeight()*sc;c.drawBitmap(b,null,new RectF(473+(28-dw)/2,t+15+(28-dh)/2,473+(28+dw)/2,t+15+(28+dh)/2),p);}text(c,n[i],526,t+25,10);mutedText(c,"가격 원전 확인 중",526,t+44,8);}}
  ...
  public boolean onTouchEvent(MotionEvent e){float x=(e.getX()-ox)/scale,y=(e.getY()-oy)/scale;switch(e.getActionMasked()){
    case MotionEvent.ACTION_DOWN:
      ...
      if(worldAdapter.inReagentShop()){requestGroundMove(x,y);return true;}
