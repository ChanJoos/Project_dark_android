...
public final class CharacterRendererAudit {
  ...
  public static boolean passes(){
    ...
    if(!"USER_CONCEPT_20260910_CHIBI_DIAGONAL".equals(CharacterRenderer.PRESENTATION_PROFILE))return false;
  ...
  public static String summary(){return "profile="+CharacterRenderer.PRESENTATION_PROFILE+",directions="+CharacterRenderer.Direction.values().length+",states="+CharacterRenderer.State.values().length+",layers="+CharacterRenderer.DRAW_ORDER.size()+",matrix="+matrix().size()+",scale="+CharacterRenderer.PLAYER_RENDER_SCALE+",headRatio="+CharacterRenderer.HEAD_TO_BODY_RATIO+",anchorY="+CharacterRenderer.LOGICAL_FOOT_ANCHOR_Y+",assets="+CharacterRenderer.ASSET_STATUS;}
