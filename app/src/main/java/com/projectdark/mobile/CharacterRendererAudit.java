...
/** Structural audit for the canonical four-direction character presentation contract. */
public final class CharacterRendererAudit {
  ...
  public static final class Case {
    ...
    Case(CharacterRenderer.Direction direction,CharacterRenderer.State state,CharacterRenderer.EffectFamily effectFamily){
      this.direction=direction;this.state=state;this.effectFamily=effectFamily;
    }
  }
  ...
  public static List<Case> matrix(){
    ...
    for(CharacterRenderer.Direction direction:CharacterRenderer.Direction.values()){
      for(CharacterRenderer.State state:CharacterRenderer.State.values())cases.add(new Case(direction,state,defaultEffect(state)));
    }
  ...
  public static boolean passes(){
    ...
    for(Case c:matrix()){
      ...
      if((c.state==CharacterRenderer.State.IDLE||c.state==CharacterRenderer.State.WALK||c.state==CharacterRenderer.State.DEAD)
          &&c.effectFamily!=CharacterRenderer.EffectFamily.NONE)return false;
    }
    return true;
  }
