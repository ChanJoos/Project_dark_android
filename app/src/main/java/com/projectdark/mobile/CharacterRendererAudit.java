...
/** Structural audit for the latest user device-playtest character presentation contract. */
public final class CharacterRendererAudit {
  ...
  public static final float USER_PLAYTEST_PLAYER_RENDER_SCALE=1.60f;
  public static final float USER_APPROVED_SHADOW_RENDER_SCALE=0.72f;
  public static final float MIN_HEAD_RATIO=0.28f;
  public static final float MAX_HEAD_RATIO=0.30f;
  ...
  public static List<Case> matrix(){
    ...
    for(CharacterRenderer.Direction d:CharacterRenderer.Direction.values())
      for(CharacterRenderer.State s:CharacterRenderer.State.values())cases.add(new Case(d,s,defaultEffect(s)));
  ...
  public static boolean passes(){
    ...
    if(!"USER_PLAYTEST_ATLAS_20260910_V5".equals(CharacterRenderer.PRESENTATION_PROFILE))return false;
    if(!CharacterRenderer.HARD_PIXEL_GRID||!CharacterRenderer.DEFAULT_ATLAS_ENABLED||!CharacterRenderer.ATTACK_ATLAS_ENABLED)return false;
    ...
    if(CharacterRenderer.ATTACK_ATLAS_COLUMNS!=4)return false;
    if(Math.abs(CharacterRenderer.PLAYER_RENDER_SCALE-USER_PLAYTEST_PLAYER_RENDER_SCALE)>.0001f)return false;
    ...
    if(CharacterRenderer.HEAD_TO_BODY_RATIO<MIN_HEAD_RATIO||CharacterRenderer.HEAD_TO_BODY_RATIO>MAX_HEAD_RATIO)return false;
    if(CharacterRenderer.HEAD_WIDTH>CharacterRenderer.SHOULDER_WIDTH+1f)return false;
    ...
    if(!CharacterRenderer.isAttackAtlasState(CharacterRenderer.State.ATTACK))return false;
    if(CharacterRenderer.isAttackAtlasState(CharacterRenderer.State.IDLE))return false;
  ...
  public static String summary(){
    return "profile="+CharacterRenderer.PRESENTATION_PROFILE+
        ",scale="+CharacterRenderer.PLAYER_RENDER_SCALE+
        ",headRatio="+CharacterRenderer.HEAD_TO_BODY_RATIO+
        ",defaultAtlas="+CharacterRenderer.DEFAULT_ATLAS_ENABLED+
        ",attackAtlas="+CharacterRenderer.ATTACK_ATLAS_ENABLED+
  ...
  private static CharacterRenderer.EffectFamily defaultEffect(CharacterRenderer.State state){
    switch(state){case CAST:return CharacterRenderer.EffectFamily.CAST;case HIT:return CharacterRenderer.EffectFamily.HIT;default:return CharacterRenderer.EffectFamily.NONE;}
  }
}
