...
public final class CharacterRendererAudit {
  ...
  public static List<Case> matrix(){
    ...
    for(CharacterRenderer.Direction direction:CharacterRenderer.Direction.values())for(CharacterRenderer.State state:CharacterRenderer.State.values())cases.add(new Case(direction,state,defaultEffect(state)));
  ...
  public static String summary(){return "directions="+CharacterRenderer.Direction.values().length+",states="+CharacterRenderer.State.values().length+",layers="+CharacterRenderer.DRAW_ORDER.size()+",matrix="+matrix().size()+",scale="+CharacterRenderer.PLAYER_RENDER_SCALE+",shadowScale="+CharacterRenderer.SHADOW_RENDER_SCALE+",anchorY="+CharacterRenderer.LOGICAL_FOOT_ANCHOR_Y+",assets="+CharacterRenderer.ASSET_STATUS;}
