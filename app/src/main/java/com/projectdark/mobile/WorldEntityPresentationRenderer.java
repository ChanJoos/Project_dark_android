...
public final class WorldEntityPresentationRenderer {
  ...
  public void draw(Canvas c,Pose pose){
    ...
    int frame=pose.state==CharacterRenderer.State.WALK?((int)(pose.walkClock*7f)%4):0;
    float walkBob=(frame==1||frame==3)?-1.2f:0f;
    float idleWave=pose.state==CharacterRenderer.State.IDLE?(float)Math.sin(pose.stateClock*4.2f):0f;
