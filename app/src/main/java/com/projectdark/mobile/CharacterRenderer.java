...
public final class CharacterRenderer {
  ...
  public void draw(Canvas c,Pose pose){
    ...
    float idleWave=pose.state==State.IDLE?(float)Math.sin(pose.stateClock*3.6f):0f; // [B] visual cadence only
    float walkSway=gait*.55f*facingX;
    float idleSway=idleWave*.28f*facingX;
    float idleBob=-Math.abs(idleWave)*.22f;
    ...
    if(pose.state==State.IDLE){float settle=Math.abs(idleWave);sw*=1f+.012f*settle;sh*=1f-.025f*settle;}
    p.setColor(pose.hitFlash?0x99ff7766:0x66000000);
    c.drawOval(new RectF(pose.x-sw,anchorY-sh,pose.x+sw,anchorY+sh),p);

    c.save();c.translate(pose.x+recoilX+walkSway+idleSway,
        anchorY-(30f*PLAYER_RENDER_SCALE)+(bob+idleBob)*PLAYER_RENDER_SCALE+recoilY);
