...
public final class CharacterRenderer {
  ...
  /** User-approved presentation-only scale. Logical/world coordinates remain unchanged. */
  ...
  /** Direction-specific future asset slots. Null means no verified crop is available yet. */
  public static final class DirectionalVisualSet {
    ...
    public String forDirection(Direction d){
      if(d==null)return null;
      switch(d){case NW:return nw;case NE:return ne;case SW:return sw;case SE:return se;default:return null;}
    }
    public boolean unresolved(){return nw==null&&ne==null&&sw==null&&se==null;}
  }

  /** Immutable presentation input. Gameplay semantics stay outside the renderer. */
  ...
  public void draw(Canvas c,Pose pose){
    ...
    int frame=pose.state==State.WALK?((int)(pose.walkClock*8f)%4):0; // [B] cadence
  ...
  private void drawBody(Canvas c,Pose pose,int frame){
    ...
    float torsoShift=side*1.2f;
    float headShift=side*1.8f;
    float nearLegX=left?5f:11f,farLegX=left?10.5f:5.5f;
    float nearArmX=left?3f:13f,farArmX=left?13f:3f;
    float castLift=pose.state==State.CAST?-6f:0f;
    float skillLift=pose.state==State.SKILL?-2.5f:0f;
    ...
    // Far leg first, then near leg: the overlap reverses with facing and keeps an isometric silhouette.
    rect(c,outline,farLegX-step,20-depth,farLegX+2.2f-step,29-depth);
    rect(c,pants,farLegX+.4f-step,20-depth,farLegX+1.8f-step,27-depth);
    rect(c,shoe,farLegX-.5f-step,27-depth,farLegX+2.8f-step,30-depth);
    rect(c,outline,nearLegX+step+kick,20+depth,nearLegX+2.4f+step+kick,29+depth);
    rect(c,pants,nearLegX+.4f+step+kick,20+depth,nearLegX+2f+step+kick,27+depth);
    rect(c,shoe,nearLegX-.5f+step+kick,27+depth,nearLegX+3f+step+kick,30+depth);

    // Narrow, shifted torso avoids a persistent front-facing rectangular body.
    rect(c,outline,4.7f+torsoShift,9f,12.3f+torsoShift,22f);
    rect(c,skin,5.6f+torsoShift,10f,11.4f+torsoShift,16.5f);

    float farSwing=pose.state==State.WALK?-step*1.7f:0f;
    float nearSwing=-farSwing;
    rect(c,outline,farArmX-1f,11f+farSwing,farArmX+1f,20f+farSwing);
    rect(c,skin,farArmX-.5f,12f+farSwing,farArmX+.5f,19f+farSwing);
    rect(c,outline,nearArmX-1f+attackReach,11f+castLift+skillLift+nearSwing,nearArmX+1f+attackReach,20f+nearSwing);
    rect(c,skin,nearArmX-.5f+attackReach,12f+castLift+skillLift+nearSwing,nearArmX+.5f+attackReach,19f+nearSwing);

    // Head is offset toward facing, producing four side-diagonal profiles instead of one frontal oval.
    p.setColor(outline);c.drawOval(new RectF(4.2f+headShift,0f,13.0f+headShift,10f),p);
    p.setColor(skin);c.drawOval(new RectF(5.0f+headShift,1f,12.2f+headShift,9f),p);
  ...
  private void drawHair(Canvas c,Pose pose){
    int hair=0xff3b251b;
    boolean left=isLeft(pose.direction),back=!isDown(pose.direction);
    float shift=left?-1.5f:1.5f;
    rect(c,hair,3f+shift,0,14f+shift,4);
    rect(c,hair,(left?2f:11f)+shift,2,(left?6f:15f)+shift,9);
    rect(c,hair,5f+shift,-1,12f+shift,2);
    if(back){
      rect(c,hair,(left?10f:3f)+shift,3,(left?14f:7f)+shift,9);
    }else{
      int eye=0xff252020;
      float eyeX=left?6f:11f;
      rect(c,eye,eyeX+shift,6,eyeX+1f+shift,7);
    }
  }

  private void drawEquipment(Canvas c,Pose pose){
    // [B] directional procedural placeholder; verified sprite binding remains PENDING_CROP.
    if(ASSET_STATUS.equals(pose.equipmentVisualRef))return;
    boolean left=isLeft(pose.direction);
    float shift=(left?-1f:1f)*1.2f;
    int shirt=pose.hitFlash?0xffffd0ca:0xffeee5d3;
    rect(c,0xff171313,4.7f+shift,9,12.3f+shift,21);
    rect(c,shirt,5.5f+shift,10,11.5f+shift,18.5f);
    rect(c,0xff3c6382,5.2f+shift,16,11.8f+shift,21);
  }
  ...
  private void drawWeapon(Canvas c,Pose pose){
    ...
    float q=phase(pose),reach=5f+7f*(q<.5f?q*2f:(1f-q)*2f);
    float handX=left?3f:14f,handY=down?15f:12f;
    p.setColor(0xffd7d2c5);p.setStrokeWidth(2);
    c.drawLine(handX,handY,handX+sx*reach,handY+sy*reach*.72f,p);
  }

  /** Owns player-local prototype effects; combat semantics remain external. */
  private void drawEffect(Canvas c,Pose pose){
    ...
    switch(family){
      case CAST:
        ...
        c.drawCircle(8+sx*2,-7+sy*1.5f,5+9*q,p);c.drawCircle(8+sx*2,-7+sy*1.5f,12-4*q,p);p.setStyle(Paint.Style.FILL);break;
      case MAGIC:
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2f);p.setColor(0xaa9d7cff);
        float mx=8+sx*5,my=6+sy*8;c.drawCircle(mx,my,4+7*q,p);
        c.drawLine(mx-sx*8,my-sy*8,mx+sx*8,my+sy*8,p);p.setStyle(Paint.Style.FILL);break;
      case THROW:
        p.setColor(0xffffd76b);c.drawCircle(8+sx*28*q,13+sy*18*q,2,p);break;
      case PUNCH:
      case KICK:
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.7f);p.setColor(0xaaffefb0);
        float cx=8+sx*11,cy=13+sy*5;c.drawArc(new RectF(cx-8,cy-8,cx+8,cy+8),left?55:205,105,false,p);p.setStyle(Paint.Style.FILL);break;
      case SKILL:
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2.6f);p.setColor(0xaa7fffa8);
        float ax=8+sx*10,ay=12+sy*7;c.drawArc(new RectF(ax-12-q*4,ay-9-q*3,ax+12+q*4,ay+9+q*3),left?40:190,130,false,p);p.setStyle(Paint.Style.FILL);break;
      case HIT:
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.5f);p.setColor(0xaaff7755);
        c.drawCircle(8-sx*4,9-sy*3,5+7*q,p);p.setStyle(Paint.Style.FILL);break;
      default:break;
    }
  }
