...
public final class F5mAdaptedPrologueQuest {
  ...
  public enum TurnInResult { COMPLETED, NOT_READY, ALREADY_COMPLETED }
  ...
  public static F5mAdaptedPrologueQuest openingFixture(){ return new F5mAdaptedPrologueQuest(OPENING_MONSTER_ID); }
  public F5mAdaptedPrologueQuest(String objectiveMonsterId){ this.objectiveMonsterId=normalize(objectiveMonsterId); }

  public String questId(){return QUEST_ID;} public String provenance(){return PROVENANCE;}
  public String balanceProvenance(){return BALANCE_PROVENANCE;} public String objectiveMonsterId(){return objectiveMonsterId;}
  public State state(){return state;} public int currentCount(){return defeated;} public int requiredCount(){return 1;}
  ...
  public AcceptResult accept(){
    ...
    state=State.ACTIVE;return AcceptResult.ACTIVATED;
  }
  ...
  public DefeatResult consume(CombatLedger.Event event){
    ...
    defeated=1;state=State.RETURN_READY;return DefeatResult.RETURN_READY;
  }

  /** Atomic quest-state side of F5M-024. Reward mutation is performed only when this returns COMPLETED. */
  public TurnInResult turnIn(){
    if(state==State.COMPLETED)return TurnInResult.ALREADY_COMPLETED;
    if(state!=State.RETURN_READY)return TurnInResult.NOT_READY;
    state=State.COMPLETED;return TurnInResult.COMPLETED;
  }

  /** Restores only valid lifecycle snapshots; impossible combinations fail closed to AVAILABLE. */
  public void restore(State restoredState,int restoredDefeated){
    if(restoredState==null){state=State.AVAILABLE;defeated=0;return;}
    if(restoredState==State.AVAILABLE){state=State.AVAILABLE;defeated=0;return;}
    if(restoredState==State.ACTIVE){state=State.ACTIVE;defeated=0;return;}
    state=restoredState;defeated=1;
  }

  void markCompletedFromTurnInTransaction(){turnIn();}
